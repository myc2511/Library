...
const getIssuedBooks = asyncHandler(async (req, res) => {
  ...
  try {
    const books = await Book.find({ "users.id": req.user.id });
...
const requestBook = asyncHandler(async (req, res) => {
  ...
  if (book.users.filter((u) => u.id === req.user.id).length !== 0) {
    res.status(400);
  ...
  try {
    const requestedBook = await Book.findOneAndUpdate(
      { _id: req.params.id },
      { $push: { requestedUsers: req.user.id } },
      { new: true }
    );
...
const cancelRequest = asyncHandler(async (req, res) => {
  ...
  if (book.requestedUsers.includes(req.user.id)) {
    const updatedBook = await Book.findOneAndUpdate(
      { _id: req.params.id },
      { $pull: { requestedUsers: req.user.id } },
      { new: true }
    );
    res.status(200);
...
const requestedBooks = asyncHandler(async (req, res) => {
  ...
  try {
    // console.log(user_id);
    const books = await Book.find({ requestedUsers: user_id });
    // console.log(books);
    res.status(200);
    res.json(books);
  } catch (error) {
...
const bookDetails = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (!book) {
      // If no book is found, respond with a 404 status code
      res.status(404);
  ...
  if (user && user.admin === false) {
    ...
    try {
      ...
      var mailOptions = {
        ...
        html: `<a href="http://localhost:8080/forgotpass/${token}">Reset Password</a>`,
      };
